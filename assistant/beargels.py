...
- 끝까지 정중한 **합니다체**('~습니다/~입니다')로 쓴다(사장님 지시 2026-09-26
  — 예전의 '편한 해요체' 규칙을 뒤집었다). '~어요/~네요/~죠' 종결은 쓰지
  않는다. 말투만 격식일 뿐 내용은 여전히 따뜻하고 구체적으로 — 공지문이
  되면 안 된다. 마무리 인사·권유("좋은 하루 보내세요", "또 주문 주세요")의
  '~세요'는 그대로 쓴다.
...
- 🚫 **'글이 없다'는 말을 하지 않는다**(사장님 지시 2026-09-26). "리뷰 글은
  없지만", "말씀은 없으셨지만", "별점만 남겨주셨는데", "사진만 남겨주셨네요"
  처럼 손님이 **안 남긴 것을 짚는 문장 금지** — 있는 것(높은 별점·사진·주문
  메뉴)에만 자연스럽게 반응한다.
...
"오늘도 좋은 하루 보내세요" · "다음에도 맛있게 구워 보내드리겠습니다" ·
"루틴처럼 찾아주시면 반갑겠습니다" · "오늘도 찾아주셔서 감사합니다"
"""
...
#    (고소하겠/고소할/고소했/고발/형사).
# ⚠️ 매칭은 **공백을 지운 본문**에 한다(아래 _is_escalation) — "머리 카락"
#    처럼 띄어 쓴 리뷰가 그대로 새는 걸 막는다(2026-08-30 감사 실측).
#    그래서 여기 키워드도 공백 없이 적는다.
ESCALATION_KEYWORDS = (
    "이물질", "머리카락", "머리카", "벌레", "곰팡이", "곰팡", "식중독", "배탈",
    "토했", "구토", "설사", "알레르기", "환불", "신고", "법적", "위생",
    # '고소했'은 뺐다 — 전 코퍼스에서 유일한 용례가 "쫄깃쫄깃하고 고소했어요"
    # (맛)였다(2026-08-30 전수 검사). 법적 위협은 아래 소송·변호사·고발이 잡는다.
    "고소하겠", "고소할", "고소접수", "고소장", "고발", "형사",
    # 2026-08-30 감사: 실제 위협 표현이 목록에 없어 complaint 로 새고 있었다
    # ("변호사와 상의해 소송 준비 중" ★1 → complaint 실측). 법적·건강·위생
    # 계열을 채운다.
    "소송", "변호사", "법률", "병원", "응급실", "진단서",
    "유통기한", "날파리", "초파리", "쉰내", "쉰맛", "시큼한냄새",
    "돈돌려", "결제취소", "보건소", "식약처",
    "상했", "상한", "철사", "역겨",
    # 비닐·플라스틱은 '이물질로 나온' 활용형만 — 베이글집 리뷰에서 이 단어는
    # 거의 전부 **포장** 얘기다(2026-08-30 전수 검사: 비닐 4건·플라스틱 1건
    # 전부 "비닐로 배달", "플라스틱 봉지" 같은 ★4~5 포장 언급 = 전부 오탐).
    "비닐이씹", "비닐씹", "비닐조각", "비닐나왔", "비닐이나왔", "비닐이들어",
    "플라스틱조각", "플라스틱이씹", "플라스틱씹", "플라스틱나왔", "플라스틱이나왔",
    # '돌이'는 활용형으로만 — '고소'와 같은 오탐 교훈이다:
    # "돌이켜보면 참 맛있었어요" ★5 가 🚨 로 막혔다(2026-08-30 감사 실측).
    "돌이나왔", "돌이들어", "돌이씹", "돌씹", "돌나왔",
)


def _is_escalation(content) -> bool:
    """사장님이 직접 봐야 하는 위협·위생·법적 표현이 있는가.

    공백을 지우고 비교한다 — 손님 글은 "머리 카락"처럼 띄어 오기도 한다.
    """
    squashed = re.sub(r"\s+", "", content or "")
    return any(k in squashed for k in ESCALATION_KEYWORDS)

# 사진/무텍스트 고평점용 클로징 변형 풀(복붙 느낌 방지 — 리뷰별로 다르게).
# ⚠️ 사장님 피드백: '~바라요/되셨으면/정성껏 준비하겠습니다' 같은 AI·공지
#    말투 금지. 말투는 정중한 합니다체(사장님 지시 2026-09-26, 해요체에서 변경).
_THANKS_VARIANTS = (
    "맛있게 드셨다니 저희도 기분이 좋습니다! 다음에 또 주문 주세요 😊",
    "이렇게 좋게 봐주시니 힘이 납니다. 다음에 또 주문 주세요!",
    "덕분에 오늘도 힘내서 일했습니다. 다음에 또 주문 주세요",
    "맛있게 드셨다니 정말 다행입니다! 다음에도 맛있게 구워 보내드리겠습니다",
    "찾아주신 것만으로 감사한데 후기까지 남겨주셔서 더 감사합니다 🥹",
    "좋게 봐주셔서 감사합니다! 다음에도 맛있게 준비하겠습니다 🐻",
)
...
_TYPE_GUIDE = {
    "praise_detail": ("리뷰에서 칭찬한 구체적 포인트(메뉴·식감·맛·서비스)를 그대로 "
                      "짚어 반응하고, 정성껏 답한다."),
    "photo_only": ("사진을 남긴 고평점이다. 남겨 준 사진에 반응하고 주문한 메뉴를 "
                   "짚어 감사한다. ⚠️ 글이 없다는 사실은 입에 올리지 않는다 — "
                   "'글은 없지만', '사진만 남겨주셨는데' 같은 문장 금지(사장님 지시 "
                   "2026-09-26). 리뷰 '내용'도 지어내 언급하지 않는다."),
    "rating_only": ("별점만 남긴 고평점이다(사진·글 없음). ⚠️ 사진이나 리뷰 글을 "
                    "절대 언급하지 마라 — 없는 걸 말하면 고객이 이상하게 느낀다. "
                    "⚠️ '별점만 남겨주셨는데', '글은 없지만'처럼 **안 남긴 것을 "
                    "짚는 말도 금지**(사장님 지시 2026-09-26) — 높은 별점 자체에 "
                    "감사하되 그 표현은 '좋게 봐주셔서' 정도로만. "
                    "대신 **주문한 메뉴**(주문 정보로 확인됨)를 짚어 그 메뉴가 어떤 "
                    "메뉴인지 한마디 곁들인다. 사장님 실제 답글은 이 유형이 가장 "
                    "길다(5문장 안팎) — '감사합니다' 한 줄로 끝내지 마라."),
    ...
    "complaint": ("서비스 리커버리 4단계로: (1)무엇이 잘못됐는지 리뷰 내용을 "
                  ...
                  "낮은 자세로. 말투는 일반 답글과 같은 정중한 합니다체로 통일. "
                  "ㅎㅎ·이모지 등 캐주얼 표현 금지. "
    ...
    if short > 0:
        ...
        if refs:
            lines.append("[참고 — 예전에 비슷한 리뷰에 답한 내용. "
                         "말투(옛 해요체 등)는 따라하지 말고 무엇을 짚었는지만 참고]")
            for ex in refs:
                lines.append("· 리뷰 \"%s\" -> %s"
...
def classify_review(review):
    ...
    반환: escalate | complaint | question | praise_detail | photo_only |
          rating_only | neutral
    """
    if _is_escalation(review.get("content")):
        return "escalate"
    rating = review.get("rating")
    content = (review.get("content") or "").strip()
    # 글 없이(또는 몇 글자만) **사진**으로 말하는 저평점 — 이물질·상태 사진일
    # 가능성이 높은데 분류기는 글만 봐서 complaint 로 흘려보냈다(2026-08-30
    # 감사). 사진 유무를 확실히 아는 쿠팡만 먼저 막는다 — 배민(_has_photo
    # None)까지 넓히면 '별점만 ★1' 전부가 직접 대응으로 넘어가 대기가 는다.
    if (rating is not None and rating <= 2 and len(content) < 10
            and _has_photo(review) is True):
        return "escalate"
...
# '손님이 안 남긴 것'을 짚는 문장 — "글은 없지만/별점만 남겨주셨는데" 류.
# 사장님이 초안에서 몇 번을 지워도 다음 초안이 또 만들어 넣어서 금지했다
# (사장님 지시 2026-09-26: "글 없다는 소리 그만 나오게 해").
_MISSING_MENTION = re.compile(
    r"글[은이도]?\s*없|말씀[은이]?\s*없|후기[는가]?\s*없|내용[은이]?\s*없"
    r"|별점만|사진만\s*남")
...
_REPLY_BANNED = (
    ...
    "역대급", "미쳤다", "인생맛집", "혜자", "대박",
    # 배민 금칙어(실측 2026-09-26, 리뷰 22368: "'새기' 키워드는 입력하실 수
    # 없습니다"로 게시 거부). '되새기며/깊이 새기고'처럼 평범한 말에 들어와
    # 게시가 통째로 막히므로 생성 단계에서 걷어낸다.
    "새기",
)
...
_BANNED_FIX = (
    ...
    # ⚠️ 치환 결과도 합니다체(사장님 지시 2026-09-26) — 예전 해요체 결과를
    #    그대로 두면 검문이 고친 자리만 말투가 튄다.
    ("되셨기를 바라며", "되었으면 하는 마음이고"),
    ("되시길 바라며", "되었으면 하는 마음이고"),
    ("되었기를 바라며", "되었으면 하는 마음이고"),
    ("바라며", "좋겠고"),
    ("바라겠", "좋겠"),
    ("기원합니다", "좋겠습니다"),
    ("되셨으면 좋겠습니다", "되었으면 좋겠습니다"),
    ("드셨길 바랍니다", "드셨으면 좋겠습니다"),
    ("드셨길 바라요", "드셨으면 좋겠습니다"),
    ("바라겠습니다", "좋겠습니다"),
    ("바랍니다", "좋겠습니다"),
    ...
    ("되었길", "되었으면 좋겠"),
    ("즐거운 한 끼", "맛있는 한 끼"),
    ("정성껏 준비하겠습니다", "더 맛있게 준비하겠습니다"),
    ("정성을 다하겠습니다", "더 맛있게 만들겠습니다"),
    ("큰 힘이 됩니다", "정말 힘이 납니다"),
    ("큰 힘이 되었습니다", "정말 힘이 났습니다"),
    ("보답하겠습니다", "더 맛있게 만들어 드리겠습니다"),
    ("보답할게요", "더 맛있게 만들어 드리겠습니다"),
    ("들러주세요", "주문 주세요"),
    ("놀러오세요", "주문 주세요"),
    ("또 오세요", "또 주문 주세요"),
    ("오시면", "주문 주시면"),
    ("와주셔서", "찾아주셔서"),
    ("와주셔", "찾아주셔"),
    # 배민 금칙어 '새기'(2026-09-26) — 긴 형태(되새기)를 먼저 바꿔야
    # '되담-' 같은 깨진 말이 안 생긴다.
    ("되새기", "다시 떠올리"),
    ("새기", "담"),
    ("역대급", "정말"),
...
def _template_reply(typ, review, author, oc, rating, max_len):
    ...
    if isinstance(oc, int) and oc > 1:
        opener = f"벌써 {oc}번째 주문입니다. 기억하고 또 찾아주셔서 정말 감사합니다."
    elif oc == 1:
        opener = "처음 주문해주셨는데 입맛에 맞으셨다니 정말 기쁩니다!"
    else:
        opener = "주문해주시고 이렇게 후기까지 남겨주셔서 감사합니다!"

    if typ == "question":
        body = (" 물어봐주신 내용은 확인해서 꼼꼼히 챙기겠습니다. "
                "편하게 여쭤봐주셔서 감사합니다!")
    else:
