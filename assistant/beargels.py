...
REPLY_PERSONA = (
    ...
    "- 닉네임 뒤 줄바꿈 후 본문. **주어진 목표 글자수에 맞춘다** — 넘기지도 "
    "말고 억지로 늘리지도 않는다. 모든 문장이 "
    "진짜 내용(구체적 메뉴 반응·감사·재방문·안부)이어야 한다.\n"
    "- 편한 구어체. 가끔 ㅎㅎ, ~, ! 나 😊🥹🐻 같은 이모지 하나 정도는 자연스럽게.\n"
    "- **말투를 하나로 통일한다**: 편한 '해요체'로 끝까지 쓴다. 격식체"
    "('~입니다/~습니다/~드립니다')나 반말('~야/~어/~지')을 섞지 않는다. "
    "예: '최고입니다'(X)→'최고예요'(O), '굽습니다'(X)→'구워요/힘나요'(O).\n"
    "- 주문 횟수 개인화: 첫 주문이면 '처음 주문해주셨는데 입맛에 맞으셨다니' 류, "
    "2~4번째면 '두 번째/세 번째'처럼 **한글 수**로 부드럽게 짚는다. "
    "5번 이상 단골은 숫자를 세지 말고 '꾸준히 찾아주셔서'·'여러 번 함께해주셔서'로 "
    "쓴다 — 사장님 실제 답글이 그렇다(숫자를 콕 집어 말하는 답글은 3.7%뿐).\n"
    ...
    "- 🔴 베이글은 본사에서 냉동 납품받아 **주문 시 그릴에 토스팅**한다. 매장에서 "
    "반죽·베이킹을 하지 않는다. '직접 반죽했어요/수제로 만들어요/갓 구운' 같은 표현은 "
    "허위라서 절대 금지. '맛있게 구워드릴게요'(=토스팅)는 사실이라 OK.\n"
    ...
    "🔸 강조어를 남발하지 마라: '진짜/너무/정말'은 답글 하나에 **한 번까지**, "
    "물결(~)도 한 번이면 충분하다. 사장님 실제 답글은 담백하다 "
    "('진짜' 사용률 직원 3.7% vs AI 22.5% — 이 격차가 직원이 매번 고치는 이유다).\n"
    "✅ 이런 느낌으로: '맛있게 드셨다니 다행이에요!', '저희가 다 기분 좋네요 ㅎㅎ', "
    "'다음에 또 주문 주세요~', '덕분에 힘나요'\n"
    "저평점이면 톤은 유지하되 변명 없이 진심으로 사과하고, '바로 확인하고 고칠게요', "
    "'다시 주문 주시면 제대로 보여드릴게요' 처럼 구체적으로 말한다.\n"
    "[좋은 예시]\n"
    "★5 첫주문 → \"KIM님,\\n처음 주문해주셨는데 입맛에 맞으셨다니 너무 좋네요! "
    "다음에 또 주문 주세요 😊\"\n"
    "★5 3번째 → \"이우님,\n세 번째 주문까지 이어서 찾아주셔서 감사해요. "
    "다음에도 맛있게 구워드릴게요 😊\"\n"
    "★5 12번째 → \"튼튼한떡갈비님,\n꾸준히 찾아주셔서 정말 감사해요. 언제 "
    "주문해주셔도 처음처럼 잘 챙겨서 보내드릴게요\""
)
...
SENTENCES_BY_KIND = {
    "rating_only": (4, 6), "praise_detail": (3, 5), "neutral": (3, 4),
    "photo_only": (2, 3), "question": (4, 6),
    "complaint": (5, 7), "escalate": (5, 7),
}

TARGET_BY_KIND = {
    "rating_only":   {"baemin": 250, "coupang": 220},
    "praise_detail": {"baemin": 150, "coupang": 205},
    "neutral":       {"baemin": 135, "coupang": 145},
    "photo_only":    {"baemin": 120, "coupang": 120},
    "question":      {"baemin": 240, "coupang": 240},
    "complaint":     {"baemin": 300, "coupang": 290},
    "escalate":      {"baemin": 300, "coupang": 290},
}


def target_len_for(kind, platform, fallback):
    """이 유형·플랫폼의 목표 글자수(모르면 기존 값)."""
    row = TARGET_BY_KIND.get(kind)
    return (row or {}).get(platform, fallback)
...
def generate_review_reply(review):
    ...
    try:
        ...
        user = (
            ...
            f"위 지침대로 답글을 써줘. **{smin}~{smax}문장**으로 쓴다 "
