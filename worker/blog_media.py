...
# 뒤에 붙는 `(팁: 찍는 요령)` 도 메모의 일부다(2026-09-17 실측: 모델이 스스로 달기 시작했고, 글 화면이 이걸
# 컷으로 못 알아봐 7자리가 사라졌다. 발행 땐 괄호 글이 네이버에 그대로 나갈 뻔했다) — 같이 잡아 같이 걷어낸다.
# group(1)=컷 설명, group(2)=팁(없으면 None). 설명 길이 한도도 120→300 으로(넘으면 통째로 안 잡혀 발행본에 남는다).
WISH_RE = re.compile(r"\[\s*📸\s*부탁\s*[:：]\s*([^\]\n]{1,300}?)\s*\]"
                     r"(?:[ \t]*\(\s*(?:팁|TIP|Tip|tip)\s*[:：]\s*([^)\n]{0,200})\))?")
