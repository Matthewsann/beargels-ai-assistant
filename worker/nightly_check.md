...
7. **답글 공부(매일):** 최근 직원이 AI 초안을 고친 쌍을 읽는다.
   ...
   - **실제 등록된 답글도 함께 공부한다**: 배민·쿠팡에 실제로 달려 있는
     ...
     ```
     이 중 규칙(정중한 합니다체 — 사장님 지시 2026-09-26 · 금지표현 · 닉네임
     등)에 맞는 좋은 답글은 lessons 의 '모범 예시'에 1~2개 추가할 수 있다.
     규칙과 어긋나는 옛 말투(편한 해요체 등)는 예시로 쓰지 않는다 —
     사실(메뉴명·상황 대응)만 참고.
   - **예시 창고 갱신(매일):** 사장님이 실제로 올린 답글이 늘어나므로,
     ...
     점검기록에 적는다(해요체·금지어가 섞인 옛 답글은 제외가 정상).
...
7-b. **릴스 문법 공부(주 1회, 월요일만):** 오늘이 월요일이 아니면 건너뛴다.
   ...
   **레퍼런스 학습으로** 올린다 — AI 가 스스로 배우는 날이 월요일이다.
   ...
   - **사실 원장 동기화도 같이:** `git log --since="7 days ago" -- knowledge/메뉴.md
     knowledge/톤앤보이스.md knowledge/브랜드.md` 로 원본이 바뀌었으면,
     `knowledge/릴스-브랜드핵심.md`(릴스 AI 전용 발췌)에 그 변경을 반영한다
     (메뉴 표기·금지어·제조 사실). 발췌가 원본과 어긋나면 허위 문구 검수가
     구멍 난다. `knowledge/릴스-출하기준.md` 의 금지어 목록과
     `sns_automation/qc.py` 상수(FALSE_CLAIMS·BANNED)도 같은 목록을 유지할 것.
