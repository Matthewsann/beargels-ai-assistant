...
- 글자 수 1,700자 이상(1,500자는 최소선 — 여유 있게).
...
- 제목은 **25자 내외, 30자를 넘기지 않는다**(검색결과에서 잘린다). 주어진 제목이 길면 대표 키워드를
  살려 줄여라. 제목에 이모지 금지.
- 분량 1,800~2,300자(사진 표시 제외).
- ★구조(90점 기준 — 채점기가 이 순서로 본다):
  ① 도입 1~2문단: 독자의 상황·고민에 공감하며 시작하고, **첫 문장에 대표 키워드**를 그 글자 그대로 넣는다.
  ...
  ④ 마무리 1문단: 핵심 한 줄 요약 + 대표 키워드 1회 + 방문·댓글을 자연스럽게 권하는 문장 하나.
...
- 강조: 절(`## `)마다 **핵심 문장 하나만** `**이렇게**` 로 감싼다(네이버에서 굵게로 들어간다 — 읽는 사람이
  훑어도 요점이 보이게). 그 이상 굵게 하지 말고, 기울임·표는 쓰지 마라(네이버가 기호를 그대로 찍는다).
...
- ★사진은 네가 고르지 않는다 — **사장님이 직접 넣는다.** 사진이 들어갈 자리마다
  `[📸 부탁: 어떤 사진]` 한 줄을 쓴다. **7~9곳(최소 6곳)** — 사진 수가 네이버 D.I.A. 점수의 핵심이라
  6곳 미만이면 감점된다.
  ...
  · 절(`## `)마다 최소 1곳. 같은 장면을 두 번 부탁하지 않는다.
