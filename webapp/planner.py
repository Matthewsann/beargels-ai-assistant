...
#
# 왜 빼는가(2026-09-06 실측): 초안 프롬프트가 35,275토큰이었고 그중 금고가
# 23,248토큰이었다. 그 안에는 오픈·마감 SOP(냉장고 온도·청소 순서), 배달
# 플랫폼 운영, 릴스 편집 문법처럼 블로그 문장에 단 한 줄도 쓰이지 않는
# 문서가 섞여 있었다. 분량이 곧 근거는 아니다 — 관계없는 지시가 많을수록
# 모델이 글의 목적에서 멀어지고, 유료 전환 시엔 그대로 돈이 된다.
#
# ★ 남기는 축: 브랜드·철학·톤앤보이스·고객·메뉴·매장정보·블로그 배운점.
#   knowledge/ 에 문서를 새로 넣으면 **자동으로 들어온다** — 블로그 글쓰기와
#   무관한 운영 문서라면 여기 이름을 추가할 것.
KNOWLEDGE_EXCLUDE = {
    "README.md",              # 색인
    "beargels_songdo.md",     # 인스타 채널 문서
    "growth_strategy.md",     # 인스타 채널 문서
    "reel_templates.md",      # 릴스 템플릿
    "릴스-편집문법.md",        # 릴스 컷·자막 규칙 — 블로그 문장과 무관
    "릴스-브랜드핵심.md",      # 브랜드.md 의 릴스용 요약(중복)
    "릴스-출하기준.md",        # 릴스 QC 기준
    "open_sop.md",            # 매장 오픈 절차(직원용)
    "close_sop.md",           # 매장 마감 절차(직원용)
    "플랫폼운영.md",           # 배민·쿠팡 운영 규칙
    "운영시스템.md",           # 내부 운영 시스템
}
...
- ★말투는 **운영자 일기체**(사장님 확정 2026-09-23): 매장을 운영하는 사장님이 오늘 매장에서 있었던 일을 적듯
  쓴다 — "오늘 매장에서는…", "아침에 반죽을 올리면서…", "저희가 준비한…", "손님들이 자주 물어보시는…".
  손님인 척("다녀왔어요", "방문했어요", "후기") 쓰지 않는다 — 거짓이 된다. 홍보문("~을 소개합니다", "~를
  자랑합니다")도 아니다. 그날의 장면·손님 반응·만드는 과정 같은 **현장의 구체적 순간**을 적는다.
- ★줄 리듬(사장님 확정 2026-09-23 — 다른 블로거 실측: 문단 평균 12~25자, 우리 글은 100자 덩어리였다):
  **한 줄에 한 문장(15~35자)**, 문장마다 줄을 바꾼다. 2~4문장이 한 덩어리가 되면 **빈 줄** 하나로 문단을 나눈다.
  한 줄이 40자를 넘기면 두 문장으로 쪼갠다. 폰 화면에서 술술 내려가야 한다.
...
- 글자 수 1,500~2,000자(1,500자는 최소선). 길게 쓰기보다 **잘게** 쓴다 — 줄 수는 60~120줄.
- ★대표 키워드 「{main_keyword}」는 **이 글자 그대로**(띄어쓰기·조사 붙이기 포함, 줄이거나 바꾸지 말 것)
  본문에 **3~5회** 넣는다: **첫 문장에 1회(사진 표시 다음 첫 글줄 — 본문 시작 200자 안에 반드시)**,
  소제목 하나에 1회, 마무리 문단에 1회, 나머지는 자연스러운 자리에.
  검색엔진은 이 정확한 문자열을 센다 — "타임스페이스 카페"처럼 앞을 떼면 0회로 친다. 6회 넘기면 도배.
- 제목은 **25자 내외, 30자를 넘기지 않는다**(검색결과에서 잘린다). 주어진 제목이 길면 대표 키워드를
  살려 줄여라. 제목에 이모지 금지.
- 분량 1,500~2,000자(사진 표시 제외).
- ★구조(90점 기준 — 채점기가 이 순서로 본다):
  ① 도입 1~2문단: 오늘 매장의 한 장면으로 시작하고, **첫 문장에 대표 키워드**를 그 글자 그대로 넣는다.
     도입 끝에 `> ` 로 시작하는 **인용 한 줄**(이 글의 한 줄 요약·훅, 20~30자)을 둔다 → 네이버 인용구가 된다.
  ② 소제목 3~4개(`## `): 소제목마다 대표 키워드 또는 세부 키워드를 하나씩 넣는다. 각 절은 250자 이상,
     "무엇을·언제·누구에게·어떻게" 중 둘 이상을 담아 정보 밀도를 채운다(뭉뚱그린 감상문 금지).
  ③ 실용 팁 절 하나: 번호 목록 3개 이상(먹는 법·주문 팁·방문 시간대·포장 등 실제로 쓸 수 있는 것).
  ④ 마무리 1문단: `> ` 인용 한 줄로 핵심 요약 + 대표 키워드 1회 + 방문·댓글을 자연스럽게 권하는 문장 하나.
     (인용 줄은 글 전체에 딱 2개 — 도입 끝, 마무리.)
- 1인칭 경험. 구체적 숫자(가격·시간)는 **확정 매장 사실·확정 메뉴표에 있는 것만** — 가격은 그 표의
  메뉴 2~4개를 이름·가격 그대로 인용해 정보 밀도를 채운다(채점기가 가장 자주 요구하는 항목).
  표에 없는 숫자는 "어떻게/언제/누구에게" 정보로 대신한다. 지어낸 숫자는 감점이 아니라 탈락이다.
- 세부 키워드는 각각 본문에 1~2회(그중 하나는 소제목에).
- 사진 자리(`[📸 부탁: …]`) 바로 앞 문장은 그 자리에 들어갈 사진에 찍힐 것을 설명한다(캡션 역할 — 채점기가 '사진 위치'를 본다).
- 강조: 절(`## `)마다 **핵심 문장 한 줄 전체**를 `**이렇게**` 로 감싼다(그 줄이 네이버에서 굵게 + 노란 형광펜이
  된다 — 읽는 사람이 훑어도 요점이 보이게). 줄 일부만 감싸지 말고 **한 줄 통째로**. 그 이상 굵게 하지 말고,
  기울임·표는 쓰지 마라(네이버가 기호를 그대로 찍는다).
- 스티커·매장 지도는 네가 쓰지 않는다 — 시스템이 절 끝마다 곰 스티커, 글 끝에 매장 지도를 자동으로 넣는다.
- ★서식(반드시 지킬 것) — 이 두 표시가 네이버 에디터에서 실제 서식으로 바뀐다:
  · 소제목은 `## 소제목 내용` 처럼 **`## ` 로 시작하는 줄**로 쓴다 → 19pt 굵은 소제목이 된다.
    ◆ ■ ▶ ● 【 】 같은 기호나 그냥 굵은 줄로 쓰면 **서식이 걸리지 않는다.**
  · 이야기가 크게 넘어가는 곳 1~2군데에 `---` 만 있는 줄을 넣는다 → 가로 구분선이 된다.
  · 본문 맨 앞에 `[제목]` `[대표 키워드]` 같은 머리말을 쓰지 않는다. 본문 글만 쓴다.
- ★사진은 네가 고르지 않는다 — **사장님이 직접 넣는다.** 사진이 들어갈 자리마다
  `[📸 부탁: 어떤 사진]` 한 줄을 쓴다. **10~14곳(최소 10곳)** — 사진 수가 네이버 D.I.A. 점수의 핵심이고
  다른 블로거는 17~36장을 쓴다(사장님 2026-09-23). 50~100자마다 사진 자리 하나 꼴로, 절마다 2~4곳.
  한 절 안에서는 서로 다른 장면(실물 클로즈업 / 과정 / 매장·좌석 / 포장 / 손님 시선)으로 나눈다.
  · 무엇을 어떻게 찍은 사진인지 **구체적으로**(피사체·각도·같이 담길 것). 이 글의 주제와 그 절의
    내용에 딱 맞아야 한다. 예: `[📸 부탁: 잠봉뵈르 베이글을 반 잘라 단면이 보이게 위에서 내려찍은 컷]`,
    `[📸 부탁: 창가 좌석에서 바깥 송도 풍경이 같이 보이는 매장 컷]`. "메뉴 사진"처럼 뭉뚱그리지 마라.
  · 부탁 바로 뒤에 `(팁: 찍는 요령 한 줄)` 을 붙인다 — 각도·거리·빛·같이 담을 것. 사장님이 매장에서 일하다
    이 한 줄만 보고 찍는다. 예: `[📸 부탁: 잠봉뵈르 단면이 보이게 반 잘라 놓은 컷] (팁: 정면 수평으로 가까이, 창가 자연광)`.
    이 괄호도 발행본에는 안 나간다.
  · 첫 문단 앞에 대표 사진 자리 1곳을 먼저 놓는다(글 전체를 대표하는 장면).
  · 절(`## `)마다 최소 2곳. 같은 장면을 두 번 부탁하지 않는다.
  · `[📷 …]` `[🎬 …]` 표시는 쓰지 않는다 — 사진함 번호·파일명을 지어내지 마라.
  · 이 메모는 사장님이 글 화면에서 사진을 넣는 자리표시일 뿐, 발행본에는 안 나간다.
...
[출력 전 스스로 확인 — 하나라도 아니면 고쳐서 내라]
1. 「{main_keyword}」가 본문 **첫 200자 안에** 그 글자 그대로 있는가?
2. 「{main_keyword}」가 본문 전체에 3~5회인가?
3. 제목이 30자 이내인가? 4. 소제목이 3개 이상이고 각각 키워드가 들었는가?
5. 번호 목록 팁이 3개 이상인가? 6. 마무리에 방문·댓글 권유 한 문장이 있는가?
8. 사진 자리가 10곳 이상인가? 9. 한 줄이 40자를 넘는 곳이 없는가(있으면 쪼갠다)? 10. `> ` 인용 줄이 2개인가?
11. 손님인 척한 문장("다녀왔어요"·"방문 후기")이 없는가?
7. 금고·확정 매장 사실에 없는 숫자·시간·가격을 쓰지 않았는가?

[출력] 아래 JSON 하나만 순수 출력(코드블록/설명 금지):
{{
  "main_keyword": "{main_keyword}",
  "sub_keywords": {sub_keywords_json},
  "title": "제목(주어진 제목 「{title}」을 바탕으로 25자 내외·30자 이내로 다듬은 것, 대표 키워드 포함)",
...
def make_draft_data(topic: str, post_type: str = "정보성", title: str = "",
    ...
    사진함 목록은 더 이상 프롬프트에 넣지 않는다(사장님 2026-09-15: "사진은 내가 넣을게").
    AI 는 사진 자리마다 `[📸 부탁: 어떤 사진]` 코멘트만 남기고, 사장님이 글 화면에서 넣는다.
    `only_rels`·`except_post_id` 는 옛 호출부 호환용으로 받기만 한다.
    """
    client, cfg, gp = _client_cfg()
    knowledge, seo = load_knowledge()
    ...
    # quality=True — 무료 **상위** 모델(gemini-flash-latest)부터 쓴다.
    # 기본값(False)은 '내부 대량 작업'용이라 하위 모델(flash-lite)을 1순위로
    # 올리는데, 블로그 본문은 손님이 읽는 글이다. 실측 2026-09-06: 하위
    # 모델이 쓴 글#2 는 `##` 소제목 0개·품질 76점, 상위 모델이 쓴 글#1 은
    # `##` 5개였다. 유료는 여전히 안 두드린다(PAID_OK 기본 false).
    # 상위 무료 한도가 마르면 자동으로 하위 모델로 내려간다.
    raw = llm.complete(user=prompt, prefer="gemini", quality=True,
