...
- 사진 표시 바로 앞 문장은 그 사진에 실제로 찍힌 것을 설명한다(캡션 역할 — 채점기가 '사진 위치'를 본다).
...
- ★사진은 위 '사진함' 목록에 **있는 것만** 쓴다. 본문에 `[📷 P07]` 처럼 번호만 적으면
  그 자리에 실제 사진이 들어간다. **7~9장(최소 6장)** — 사진 수가 네이버 D.I.A. 점수의 핵심이라
  6장 미만이면 감점된다. **목록에 없는 번호를 지어내지 마라.**
  ...
  · 어떤 절에 **목록의 어떤 사진도 안 어울리면** 번호를 억지로 쓰지 말고 그 자리에
    `[📸 부탁: 어떤 사진이 있으면 좋을지 한 줄]` 을 쓴다(예: `[📸 부탁: 닭가슴살 샐러드 위에서 내려찍은 컷]`).
    사장님이 그 사진을 찍어 그 자리에 넣는다. 발행본에는 안 나간다.
