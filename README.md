# 베어글스 송도 인스타그램 SNS 자동화

주제별로 정리한 사진/영상을 **내가 부를 때** 릴스/게시물로 만들어주는 봇입니다.
(자동 폴링이 아니라 텔레그램에서 주제명을 부르면 그 폴더를 분석합니다.)

## 파이프라인 흐름 (폴더 기반 on-demand)

```
1. 구글 드라이브 "콘텐츠 생성 > [주제명]" 폴더에 사진/영상 정리
        │  텔레그램에 주제명 전송 (예: "소금빵 신메뉴")   ← 내가 부를 때 시작
        ▼
2. Claude — 폴더의 미디어 분석 → 릴스/게시물 캡션 + 해시태그(알고리즘 최적화)
        ▼
3. 텔레그램 승인 요청 (미리보기 + [✅ 승인 / ✏️ 수정 / ❌ 취소])
        │
        ├─ ✅ 승인 → Buffer 큐 → 인스타그램 릴스/게시물 발행
        ├─ ✏️ 수정 → 🤖 AI 수정  또는  ⌨️ 직접 입력 → 다시 승인 요청
        └─ ❌ 취소 → 발행 안 함
```

- 영상이 있으면 **릴스**로, 사진만 있으면 게시물로 처리합니다.
- 브랜드 톤·인스타 알고리즘 지침은 `knowledge/beargels_songdo.md`를 근거로 합니다 (편집 가능).
- `/목록` 을 보내면 작업 가능한 주제 폴더 목록을 보여줍니다.
- 처리 상태는 Supabase `sns_posts` 테이블에 기록됩니다.
...
3. `supabase/migrations/003_topic_folders.sql` — 폴더(주제) 기반 처리용 컬럼
