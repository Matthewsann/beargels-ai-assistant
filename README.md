...
네이버 블로그 운영은 **직원 웹의 `/blog`(기획실)** 에서 합니다 — 무료 Gemini
(`llm.complete`)로 글감 추천→초안→퇴고→네이버 임시저장까지 자동, 최종
발행·예약만 사람이 네이버에서 직접 누릅니다(CLAUDE.md의 블로그 목표 참고).

- 상위 노출 로직(C-Rank·D.I.A.)은 [`네이버-SEO-지식.md`](네이버-SEO-지식.md)에 정리, AI가 학습해 반영
- ⚠️ `automation/` 폴더의 옛 생성기(generate_post·stockpile)는 **퇴역**
  (2026-08-30 비용 감사 — 유료 Claude 를 직접 불러 실행하면 무조건 과금이라
  차단해 둠). 실행하지 마세요. 웹 기획실이 같은 일을 무료로 합니다.
