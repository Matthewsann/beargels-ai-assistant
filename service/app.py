...
def todo(path_key):
    ...
    ⚠️ 이 페이지군(todo/history/review)의 궁극적 목표(사장님 확정
    2026-08-28) — 이 화면을 바꿀 때마다 이 기준으로 판단한다:
        관리자가 배민·쿠팡 리뷰를 플랫폼에서 하나하나 직접 답글 쓰지 않고,
        한 곳에서 AI 도움을 받아 빠르게 등록하게 한다.
        목표치: 리뷰 10건 답글 ≈10분 → **이 페이지로 ≈1분**.
        문제 될 만한 리뷰(에스컬레이션 등)는 꼼꼼히 걸러 관리자가 직접
        확인 후 게시되게 한다. 리뷰 속 고객 요청·반응은 놓치지 않고
        전 직원에게 쉽게 전파되게 한다(→ customer_requests.py).
    자세한 배경은 CLAUDE.md '리뷰 답글 페이지의 목표' 절 참고.
...
def instagram_published(path_key):
    ...
    안 눌러도 집 PC 가 하루 한 번 내 계정 게시물을 읽어 알아챈다 — 이 버튼은
