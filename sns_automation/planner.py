...
def _sales() -> str:
    """지금 실제로 팔리는 메뉴 — 인스타 기획에서는 '어떤 메뉴로 찍을까'의 보조 근거.

    주제 선정 기준은 인스타 데이터(계정 성과·해시태그 시장)가 위다(사장님 2026-09-17).
    """
    try:
...
def _ideas_system() -> str:
    """인스타 기획 프롬프트 — **인스타 기준으로만** 판단한다(사장님 확정 2026-09-17).

    예전엔 네이버 유입 검색어·네이버 키워드 실측이 이 프롬프트에 들어가고
    제안마다 블로그 키워드까지 뽑게 했다(두 채널 관통 브리프, 2026-09-04).
    사장님이 "인스타용은 철저히 인스타그램 기준으로" 하라고 해 전부 뺐다.
    블로그 기획은 /blog 의 자체 흐름(키워드 → 주제 → 초안)이 맡는다.
    """
    from datetime import date
    sales = _sales()
    sales_block = ""
    if sales:
        sales_block = ("[참고 — 지금 잘 팔리는 메뉴 (주제 기준이 아니라 '어떤 메뉴로 찍을까'의 보조)]\n"
                       + sales)
    return (
        "너는 베이글 카페 '베어글스 송도점'의 릴스 편집 전문가이자 인스타그램 마케팅 파트너다.\n"
        "사장님이 매장에서 폰으로 2~3분 안에 찍을 수 있는 릴스 촬영 아이디어를 낸다.\n"
        "원칙:\n"
        "· **판단 기준은 인스타그램이다.** 네이버 검색이나 블로그 사정은 고려하지 않는다.\n"
        "· **근거의 우선순위**: ①내 계정에서 실제로 먹힌 훅·소재(좋아요·댓글)\n"
        "  ②동네·카테고리 해시태그 인기글의 문법 ③릴스 편집 트렌드 ④계절·날짜\n"
        "  ⑤지금 잘 팔리는 메뉴(어떤 메뉴로 찍을지 고를 때만). 아래 블록이 서로\n"
        "  어긋나면 위쪽을 따른다. 잘 팔린다고 인스타에서 먹히는 건 아니다.\n"
        ...
        "· 성과 데이터가 있으면 먹힌 각도(반전형)를 우선하고, 약했던 패턴은 피한다.\n"
...
async def plan_from_topic(topic: str) -> list[dict]:
    """사장님이 직접 정한 주제 → 촬영 가이드(훅·샷)를 붙인 기획 1개.

    주제는 사장님 것이니 바꾸지 않는다. 파트너의 몫은 '그 주제를 어떻게 찍을지'다
    (인스타 화면 [＋ 내가 정한 주제], 2026-09-17).
    """
    user = (
        "[사장님이 정한 주제]\n" + topic.strip()[:300] + "\n\n"
