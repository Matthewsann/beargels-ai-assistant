...
값은 **단가_합계(부가세 포함)** 를 쓴다 — 실제로 통장에서 나가는 돈이다.
매장 판매가도 부가세 포함이라, 양쪽을 같은 기준으로 둬야 원가율이 말이 된다
(사장님 방침 2026-08-16). 예전에는 공급가로 넣었는데, 재료만 세전이고 판매가는
세포함이라 원가율이 실제보다 9% 낮게 나왔다.
"""
...
def main():
    ...
    for r in rows:
        ...
        cost = _num(r.get("단가_합계")) or _num(r.get("공급가"))
