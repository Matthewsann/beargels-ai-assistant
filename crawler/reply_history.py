...
⚠️ 말투 주의: 과거 답글 상당수는 이전(AI/템플릿) 방식이라 '~바라요/정성껏
   ...
   '말투' 학습용이 아니다. 말투는 REPLY_PERSONA(정중한 합니다체)를 따른다.
...
- 전체 판매 메뉴는 {menu_count}종이다(목록은 답글 작성에 필요 없어 넣지 않는다).
...
말투는 REPLY_PERSONA(정중한 합니다체)를 따르고, 여기선 **사실만** 가져온다.
"""
...
def _write_context(data):
    ...
    # 메뉴 '목록'은 더 이상 넣지 않는다 — 198개 이름을 나열하느라 프롬프트의
    # 절반(5,100자)을 잡아먹었고 정작 이 리뷰와는 상관이 없었다. 주문한 메뉴의
    # 사실은 menu_facts_for() 가 건별로 넣는다(2026-08-25).
    menu_lines = _menu_lines_from_master() or ''
    menu_count = len([l for l in menu_lines.split(chr(10)) if l.strip()]) or '여러'
    
    (REF_DIR / "reply_context.md").write_text(
        _CONTEXT_HEADER.format(menu_count=menu_count), encoding="utf-8")
...
def _write_markdown(data):
    lines = ["# 베어글스 리뷰·답글 내역 (자동 생성)", "",
             ...
             "> 섞여 있으니 **말투는 따라 하지 말고** 정중한 합니다체로 씁니다.", ""]
